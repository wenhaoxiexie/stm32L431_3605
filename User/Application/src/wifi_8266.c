...
uint8_t wifi_upload_all_info(void)
{
	...
	if(reStatus==2)
	{
		...
		HAL_WIFI_STATUE_CHECK(HAL_OK, wifi_trans_recev_command((uint8_t*)AT_TCP_SERVER,sizeof(AT_TCP_SERVER),SERVER_KEYWORDS,NULL,40));
	}
	else if(reStatus==3)
	{
...
HAL_StatusTypeDef wifi_trans_recev_command(uint8_t* command,uint16_t cLen,const char* itemStr,uint8_t* reBuff,uint16_t timeout)
{
	...
	HAL_WIFI_STATUE_CHECK(HAL_OK,HAL_UART_Transmit(&huart3,command,cLen,1000));
