...
static uint8_t factory_test(void)
{
	...
	do{
		if(get_cur_number.key_prees)
		{
			...
			if(wifi_get_mac(mac)==HAL_OK&&strcmp((char*)mac,"FFFFFFFFFFFFFFF")!=NULL)
			{
...
uint8_t finger_login(uint8_t* permision)
{
    ...
    pPakege[3]= 0;
    pLen=4;
		reValue = fp_tarns_recv_command(PS_Compare,pPakege,&pLen, pPakege,&pResult,2000);
		debug_printf("reValue: %d  %d [%x:%x]\r\n",reValue,pResult,pPakege[2],pPakege[3]);
		if(reValue!=FP_OK)
		{
				goto ERROR;
		}	
		else{
				if(pPakege[2]==0&&pPakege[3]==0)
					goto ERROR;
				...
				*permision =finger_flash[i].user_permision;
				...
				{
						wifi_flash[wifi_index].exist=1;
						sprintf(wifi_flash[wifi_index].upload_info,"%d%%%d%%%s%%",WARN_UPLOAD,ANTI_HIJACK_WARN,lock_time);
						if(wifi_flash->index<99)
							wifi_flash->index++;
				}else{
...
void SYSCLKConfig_STOP(uint16_t GPIO_Pin)
{
  ...
  if (HAL_RCCEx_PeriphCLKConfig(&PeriphClkInit) != HAL_OK)
  {
	...
	if(get_cur_info->cur_index_number>=1&&(GPIO_Pin==GPIO_PIN_3||GPIO_Pin==GPIO_PIN_4||GPIO_Pin==GPIO_PIN_8||GPIO_Pin==GPIO_PIN_12))
		display_menu();
...
void sleep_mode(void)
{
	...
	wifi_power_off();
	// 指纹断电
	debug_printf("##########FPPower_Pin:  %d\r\n",HAL_GPIO_ReadPin(FPPower_GPIO_Port,FPPower_Pin));
...
//  关闭串口3
  __HAL_UART_DISABLE(&huart3);
	...
	GPIO_InitStructure.Pin = GPIO_PIN_0; // 电源开关 外部下拉
	GPIO_InitStructure.Mode = GPIO_MODE_INPUT;
	GPIO_InitStructure.Pull = GPIO_PULLDOWN;
	HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);
	
	GPIO_InitStructure.Pin = GPIO_PIN_All;
	GPIO_InitStructure.Pin &= (~(GPIO_PIN_0|GPIO_PIN_1|GPIO_PIN_9|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_12));// 中断引脚
	GPIO_InitStructure.Mode = GPIO_MODE_ANALOG;
	GPIO_InitStructure.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(GPIOA, &GPIO_InitStructure);
